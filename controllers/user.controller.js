...
const generateToken = (user) => {
    return jwt.sign({ id: user.id, email: user.email }, "d4f5efe5f4e5f", { expiresIn: "2h" })
}
...
export const updateUser = async (req, res) => {
    try {
        ...
        if (checkNewPass) {
            return res.status(404).json("New password cannot be the same as the old password")
        }
        const hashnewPass = await bcrypt.hash(newPassword, 10)

        const userData = await db.User.update({ password: hashnewPass }, { where: { email } })
